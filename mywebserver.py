...
class FlaskAppWrapper(MyLog):
    ...
    def __init__(self, name=__name__, static_url_path='', log=None, shutter=None, schedule=None, config=None):
        ...
        self.add_endpoint(endpoint='/cmd/<command>', endpoint_name='cmd', handler=self.processCommand,
                          methods=['GET', 'POST'])
    ...
    def deleteShutter(self, params):
        ...
        if id not in self.config.Shutters:
            ...
            self.config.WriteValue(str(id),
                                   self.config.Shutters[id]['name'] + ",False," + self.config.Shutters[id]['duration'],
                                   section="Shutters")
            self.config.ShuttersByName.pop(self.config.Shutters[id]['name'], None)
